...
def copy_template(template_dir: Path, dest: Path, skip_agents: bool = True, include_tool_plugins: bool = False):
    """
    ...
    for src_path in sorted(template_dir.rglob("*")):
        ...
        # Core setup should not install optional tool plugin assets.
        if not include_tool_plugins:
            if rel.name in {"Dashboard.html", "export_dashboard.py"}:
                continue
...
def copy_asset_tree(src_root: Path, dest: Path) -> list[str]:
    """Copy provider vault assets into the vault without overwriting user files."""
    copied: list[str] = []
    if not src_root.exists():
        return copied
    for src_path in sorted(src_root.rglob("*")):
        rel = src_path.relative_to(src_root)
        if "__pycache__" in rel.parts or src_path.suffix == ".pyc":
            continue
        dest_path = dest / rel
        if src_path.is_dir():
            dest_path.mkdir(parents=True, exist_ok=True)
...
def write_setup_report(vault_path: Path) -> Path:
    ...
    lines.extend([
        "",
        "## Optional local automation plugin (launchd)",
        "",
...
def main():
    ...
    parser.add_argument("--core-only", action="store_true", help="Install only the LLM-GTD core vault contract and agent instructions; skip bundled tool plugins")
    parser.add_argument("--with-bundled-tools", action="store_true", help="Install the bundled local tool recipe explicitly")
    parser.add_argument("--install-dashboard-app", action="store_true", help="Install the optional Dashboard.app plugin")
    parser.add_argument("--install-automation", action="store_true", help="Install optional launchd local automation")
    ...
    parser.add_argument("--im-platform", choices=IM_PLATFORMS + ["none"], default="none", help="Document sync IM platform")
    ...
    if args.with_bundled_tools:
        args.install_dashboard_app = True
        args.install_automation = True
        if not args.skip_quickcapture:
            args.install_quickcapture = True

    if args.core_only:
        args.with_bundled_tools = False
        args.install_dashboard_app = False
        args.install_automation = False
        args.install_quickcapture = False

    if not args.install_dashboard_app:
        args.no_app = True
    if not args.install_automation:
        args.skip_automation = True
    ...
    selected_plugins = {
        "dashboard_app": args.install_dashboard_app and not args.no_app,
        "launchd": args.install_automation and not args.skip_automation,
        "quickcapture": args.install_quickcapture and not args.skip_quickcapture,
    }
    has_selected_plugins = any(selected_plugins.values())
    needs_dashboard_assets = args.install_dashboard_app or args.install_automation or args.with_bundled_tools
    needs_vault_script_assets = args.install_automation or args.with_bundled_tools
    setup_recipe = "bundled_tools" if args.with_bundled_tools else ("selected_plugins" if has_selected_plugins else "core_only")
    ...
    copy_template(TEMPLATE_DIR, vault_path, include_tool_plugins=False)
    if needs_dashboard_assets:
        copied = copy_asset_tree(PLUGIN_DIR / "dashboard" / "vault-assets", vault_path)
        if copied:
            print(f"  ✓ Dashboard provider assets copied ({len(copied)} file(s))")
    if needs_vault_script_assets:
        copied = copy_asset_tree(PLUGIN_DIR / "vault-scripts" / "vault-assets", vault_path)
        if copied:
            print(f"  ✓ Vault script provider assets copied ({len(copied)} file(s))")
    if features["doc_sync"]:
        copied = copy_asset_tree(PLUGIN_DIR / "online-docs" / "vault-assets", vault_path)
        if copied:
            print(f"  ✓ Online-doc provider assets copied ({len(copied)} file(s))")
    ...
    dashboard_status = "ok" if has_selected_plugins and (vault_path / "Dashboard.html").exists() else "skipped"
    update_setup_state(
        vault_path,
        steps={"init_vault": "ok"},
        capabilities={"dashboard": dashboard_status},
    ...
    if platform.system() == "Darwin" and not args.no_app:
        ...
        update_setup_state(vault_path, capabilities={"dashboard_app": "skipped"})
    ...
    agent_cron_status = "pending" if args.with_bundled_tools else "skipped"
    im_docs_status = "pending" if features["doc_sync"] else "skipped"
    update_setup_state(
        vault_path,
        steps={
            "install_local_tools": "ok" if has_selected_plugins else "skipped",
            "register_agent_cron": agent_cron_status,
            "connect_im_docs": im_docs_status,
            "onboard": "pending",
        },
        capabilities={"im_docs": im_docs_status, "agent_cron": agent_cron_status},
    )
    if args.with_bundled_tools:
        try:
    ...
    skipped_plugin_components = set()
    if not has_selected_plugins:
        skipped_plugin_components.update({"dashboard", "vault_scripts", "dashboard_app", "launchd", "quickcapture"})
    if not args.with_bundled_tools:
        skipped_plugin_components.add("agent_cron_guide")
    if not features["doc_sync"]:
        skipped_plugin_components.add("doc_sync_protocol")
    ...
    for component in component_helpers.load_manifest(REPO_ROOT):
        ...
        component_status = "skipped" if component["id"] in skipped_plugin_components else "ok"
        component_helpers.mark_component_applied(
            vault_path,
            component,
            source_hash,
            status=component_status,
            details={"setup_baseline": True, "setup_recipe": setup_recipe},
        )
    ...
    capabilities = state.get("capabilities", {})
    print("  Core:")
    print(f"    Vault:          {status_label(capabilities.get('vault', 'unknown'))}")
    print(f"    Instructions:   {status_label('ok' if agents_dest.exists() else 'missing')}")
    print()
    print("  Optional tool plugins / integrations:")
    print(f"    Dashboard view: {status_label(capabilities.get('dashboard', 'unknown'))}")
    print(f"    Dashboard.app:  {status_label(capabilities.get('dashboard_app', 'unknown'))}")
    ...
    if has_selected_plugins:
        step += 1
        print(f'  {step}. Optional tool plugins were selected in this setup.')
        print(f'     Run doctor with plugin checks if you want deeper verification:')
        print(f'     python3 {REPO_ROOT}/tools/setup/doctor.py --vault "{vault_path}" --check-cron --check-quickcapture')
        print()
    else:
        step += 1
        print(f'  {step}. Optional tool plugins can be enabled later if useful:')
        print(f'     Dashboard.app, QuickCapture, local automation, scheduler, or online docs')
        print(f'     See {REPO_ROOT}/docs/tool-plugins.md')
        print()
    step += 1
