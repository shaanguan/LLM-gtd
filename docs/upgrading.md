...
**Default path for users:** say `升级 GTD` / `upgrade gtd` to the Agent. The skill runs `tools/setup/upgrade.py`, which compares `tools/setup/components.json` against `.llm-gtd/component-state.json` and applies only changed components.
...
1. `python3 tools/setup/upgrade.py --vault "$GTD_VAULT" --check --json`
...
4. `python3 tools/setup/doctor.py --vault "$GTD_VAULT" --check-updates --check-cron --json`
...
```bash
python3 tools/setup/upgrade.py --vault "$GTD_VAULT" --check
```

JSON output:

```bash
python3 tools/setup/upgrade.py --vault "$GTD_VAULT" --check --json
```

Doctor can also remind you:

```bash
python3 tools/setup/doctor.py --vault "$GTD_VAULT" --check-updates
```
...
```bash
python3 tools/setup/upgrade.py --vault "$GTD_VAULT" --apply --components dashboard_app --force
python3 tools/setup/upgrade.py --vault "$GTD_VAULT" --apply --components agent_instructions
python3 tools/setup/upgrade.py --vault "$GTD_VAULT" --apply --components gtd_knowledge_base
```
...
| `.llm-gtd/knowledge-link.txt` (vault) | Pointer to the repo `vaults/knowledge/gtd` asset; the knowledge base is not copied into `00~07` user data |

If `.llm-gtd/version` is older than repo `VERSION`, run `tools/setup/upgrade.py --check --json` and apply changed components.
